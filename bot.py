...
def main_menu_kb():
    ...
    quiz_label = f"📝 Пройти тест ({q_count} питань)" if q_count else "📝 Пройти тест"
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=quiz_label, callback_data="quiz_start")],
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"👋 Привіт, {name}!\n\n"
        "Я довідник продавця Фокстрот. Допоможу швидко знайти інформацію "
        "по послугам і пакетам та перевірити свої знання тестом.\n\n"
        "Обери розділ нижче 👇",
